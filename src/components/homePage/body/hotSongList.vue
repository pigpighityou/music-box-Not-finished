...
(async () => {
  try {
    ...
    hotSongsList.songs = hotSongsAPI.data;
...
const getSong = async (id) => {
  try {
    hotSongsAPI = await getListSong(id);
    hotSongsList.songs = hotSongsAPI.data;
    console.log("okDetail", hotSongsList.songs);
  } catch (err) {
    console.log(err);
  }
};
...
const imgList = computed(() => {
  if (hotSongsList.songs.playlists) {
    return hotSongsList.songs.playlists;
  } else {
...
const foreignImgList = computed(() => {
  if (hotSongsList.songs.playlists) {
    return hotSongsList.songs.playlists.filter((item) => {
